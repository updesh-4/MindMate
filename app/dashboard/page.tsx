...
export default function Dashboard() {
  ...
  // All state declarations together at the top
  ...
  // Modify the loadActivities function to set the loaded flag
  ...
  // Update stats when activities are loaded
  useEffect(() => {
    if (activitiesLoaded) {
  ...
  // Update insights when activities are loaded
  useEffect(() => {
  ...
  // Fixed function to fetch daily stats - now uses existing data
  const fetchDailyStats = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        therapySessionsCount = sessions.length;
      } catch (error) {
        console.warn("Could not fetch therapy sessions:", error);
        therapySessionsCount = activities.filter((a) => a.type === "therapy").length;
      }
      ...
      setDailyStats({
        moodScore: averageMood,
        completionRate: 100,
        mindfulnessCount: therapySessionsCount,
        totalActivities: todaysActivities.length,
        lastUpdated: new Date(),
      });
    } catch (error) {
  ...
  // Fetch stats when activities are loaded
  useEffect(() => {
    if (activitiesLoaded) {
      fetchDailyStats();
    }
                  ...
                  {wellnessStats.map((stat) => (
                    ...
                    >
                      ...
                      <p className="text-2xl font-bold mt-2">
                        {isLoadingActivities && stat.title === "Total Activities" 
                          ? "Loading..." 
                          : stat.value}
                      </p>
